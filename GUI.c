...
void writeMemory(char memory[],int memoryAmount) { //aktualizuje pamiec
	int i = 0,j=currentMemStart*8,z=0,a=0;
	int* tab = (int*)malloc(memoryAmount / 8 * sizeof(int));

	for (i = 0; i < memoryAmount / 8; i++) {
		tab[i] = 0;
	}
	for (i=0; i < memoryAmount / 8;a++) {
		tab[i] = 1;
		i += ROW_ARRAY[a].amount;
	}
	a = 0;
	for (i = 0; i < currentMemStart; a++) {
		i += ROW_ARRAY[a].amount;
	}
	for(i = 0; i < 30; i++) {
	...
	for (i=currentMemStart; i < min(30+currentMemStart, memoryAmount / 8); i++) {
		mvwprintw(memory_content, i-currentMemStart, 1, "%3d:", j / 2);
		if (tab[i]) {
			mvwprintw(memory_content, i - currentMemStart, 5, " %s", ROW_ARRAY[a].label);
			a++;
		}
...
void printStaticText(char* filename, char* filenameM) { //drukuje caly tekst ktory sie nie bedzie zmienial
	...
	mvwprintw(top_content, 2, 0, "'Q'-wyjdz z programu, 'N'-nastepny krok");
...
void changeMemStart(int n) {
	if (n == 1) currentMemStart = max(0, currentMemStart - 1);
	if (n == 2) {
		if(memRowAmount1>30) currentMemStart = min(memRowAmount1 - 30, currentMemStart + 1);
	}
}